...
-spec(start_channel(inet:socket(), {inet:ip_address(), inet:port()}) -> {ok, pid()}).
start_channel(Sock, Peer) ->
    supervisor:start_child(?MODULE, [Sock, Peer]).

init([]) ->
    {ok, {{simple_one_for_one, 0, 1},
          [{caop_channel, {emqtt_caop_channel, start_link, []},
              temporary, 5000, worker, [emqtt_caop_channel]}]}}.
