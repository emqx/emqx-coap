...
case01_create(_Config) ->
    ...
    TopicInfo = [{TopicInPayload, {timer_state, MaxAge1, _, _, _}, CT1, _ResPayload, undefined}] = emq_coap_ps_topics:lookup_topic_info(TopicInPayload),
    ...
    ?assertEqual([<<"/ps/topic1">>] ,LocPath),
    [{TopicInPayload, {timer_state, MaxAge2, _, _, _}, CT2, _ResPayload, undefined}] = emq_coap_ps_topics:lookup_topic_info(TopicInPayload),
...
case02_create(_Config) ->
    ...
    [{FullTopic, {timer_state, MaxAge2, _, _, _}, CT2, _ResPayload, undefined}] = emq_coap_ps_topics:lookup_topic_info(FullTopic),
...
case01_publish_post(_Config) ->
    ...
    [{FullTopic, {timer_state, MaxAge, _, _, _}, CT2, <<>>, undefined}] = emq_coap_ps_topics:lookup_topic_info(FullTopic),
    ...
    ?LOGT("Reply =~p", [Reply2]),
    {ok,changed, _} = Reply2,
    TopicInfo = [{FullTopic, {timer_state, MaxAge, _, _, _}, CT2, PubPayload, _TimeStamp}] = emq_coap_ps_topics:lookup_topic_info(FullTopic),
...
case02_publish_post(_Config) ->
    ...
    {ok,created, #coap_content{location_path = LocPath}} = Reply,
    ?assertEqual([<<"/ps/topic1">>] ,LocPath),
    [{Topic, {timer_state, MaxAge, _, _, _}, CT, Payload, _TimeStamp}] = emq_coap_ps_topics:lookup_topic_info(Topic),
    ?assertEqual(60, MaxAge),
    ?assertEqual(<<"42">>, CT),
    ...
    ?LOGT("Reply =~p", [Reply1]),
    {ok,changed, _} = Reply1,
    [{Topic, {timer_state, MaxAge, _, _, _}, CT, NewPayload, _}] = emq_coap_ps_topics:lookup_topic_info(Topic),
...
case01_publish_put(_Config) ->
    ...
    {ok,created, #coap_content{location_path = LocPath1}} = Reply1,
    ?assertEqual([<<"/ps/maintopic/topic1">>] ,LocPath1),
    [{FullTopic, {timer_state, MaxAge, _, _, _}, CT2, <<>>, _TimeStamp}] = emq_coap_ps_topics:lookup_topic_info(FullTopic),
    ?assertEqual(60, MaxAge),
    ?assertEqual(<<"42">>, CT2),
    ...
    ?LOGT("Reply =~p", [Reply2]),
    {ok,changed, _} = Reply2,
    [{FullTopic, {timer_state, MaxAge, _, _, _}, CT2, PubPayload, _TimeStamp1}] = emq_coap_ps_topics:lookup_topic_info(FullTopic),
...
case02_publish_put(_Config) ->
    ...
    ?LOGT("Reply =~p", [Reply1]),
    {ok,changed, _} = Reply1,
    [{Topic, {timer_state, MaxAge, _, _, _}, CT, NewPayload, _TimeStamp1}] = emq_coap_ps_topics:lookup_topic_info(Topic),
...
case01_subscribe(_Config) ->
    ...
    {ok,created, #coap_content{location_path = [LocPath]}} = Reply,
    ?assertEqual(<<"/ps/topic1">> ,LocPath),
    TopicInfo = [{Topic, {timer_state, MaxAge1, _, _, _}, CT1, _ResPayload, _TimeStamp}] = emq_coap_ps_topics:lookup_topic_info(Topic),
    ?LOGT("lookup topic info=~p", [TopicInfo]),
    ?assertEqual(60, MaxAge1),
    ?assertEqual(<<"42">>, CT1),
...
case02_subscribe(_Config) ->
    ...
    [{Topic, {timer_state, MaxAge1, _, _, _}, CT, Payload, TimeStamp}] = emq_coap_ps_topics:lookup_topic_info(Topic),
    ?assertEqual(60, MaxAge1),
    ?assertEqual(<<"42">>, CT),
